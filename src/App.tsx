...
import { DogProfile, TrainingLog, Milestone, ChatMessage, LogStatus, CustomSkillItem } from "./types";
...
const ALL_SKILLS = useMemo(() => [...PRESET_SKILLS, ...customSkills.map(s => s.name)], [customSkills]);
...
export default function App() {
  ...
  const handleAddCustomSkill = (e: React.FormEvent) => {
    ...
    setCustomSkills((prev) => [...prev, { id: "cs-" + Date.now(), name: newCustomSkillName.trim(), description: "User-defined custom skill." }]);
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          history: chatHistory,
          customLlmConfig: llmConfig,
          customSkills: ALL_SKILLS // Pass all skills, including custom ones
        })
      });
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-[#fbfcff] border border-blue-100 rounded-2xl p-5 shadow-xs">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
              {ALL_SKILLS.slice(0, 4).map((skill) => (
                <button
                  key={skill}
                  onClick={() => {
            ...
            <div className="p-5 overflow-y-auto flex-1 space-y-4 bg-neutral-50/25">
              ...
              {llmConfig.enabled && (
                <div className="space-y-3.5 animate-fadeIn p-4 border border-purple-100 bg-purple-50/10 rounded-xl">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-[10px] font-mono font-bold tracking-wider text-neutral-400 uppercase border-b pb-1">➕ CUSTOM TRAINING COMMANDS</h4>
                    <div className="space-y-2">
                      {customSkills.length === 0 && <p className="text-xs text-neutral-500 italic">No custom skills added yet.</p>}
                      {customSkills.map((skill) => (
                        <div key={skill.id} className="flex items-center justify-between bg-white border border-neutral-200 rounded-lg p-2.5 shadow-2xs">
                          <span className="text-xs text-neutral-700 font-medium">{skill.name}</span>
                          <button
                    ...
                    <form onSubmit={handleAddCustomSkill} className="flex gap-2 mt-2">
                      <input
                        type="text"
                        placeholder="Add new skill (e.g., 'Fetch')"
                        value={newCustomSkillName}
                        onChange={(e) => setNewCustomSkillName(e.target.value)}
                        className="flex-1 text-xs px-3 py-2 bg-white border border-neutral-200 rounded-lg focus:outline-none focus:border-purple-400"
                      />
                      <button
                        type="submit"
                        className="px-3.5 py-1.5 bg-blue-600 border border-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors"
                      >
